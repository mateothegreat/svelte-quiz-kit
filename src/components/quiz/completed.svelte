...
<CardHeader class="text-center">
  {#if passed}
    <Trophy class="h-16 mx-auto text-yellow-500 w-16" />
  {:else}
    <Target class="h-16 mx-auto text-blue-500 w-16" />
  {/if}
...
<CardContent class="space-y-6">
  <div class="text-center">
    <div class="font-bold mb-2 text-4xl">{score}%</div>
  ...
  <div class="space-y-2">
    ...
    {#each config.questions as question, index}
      <div class="bg-muted flex items-center justify-between p-2 rounded">
        <span class="text-sm">Question {index + 1}</span>
        {#if isAnswerCorrect(question.id, answers[question.id])}
          <CheckCircle2 class="h-5 text-green-600 w-5" />
        {:else}
          <XCircle class="h-5 text-red-600 w-5" />
        {/if}
      </div>
    {/each}
  </div>
